...
function showPage(id) {
  ...
  const _topSearch = document.getElementById('topSearch');
  ...
  newItems.forEach((p, idx) => {
    ...
    div.innerHTML = `
      ...
      <div class="prod-body">
        <div class="prod-name" onclick='openProdModal("${esc(p._id)}")'>${p.name}</div>
        ${priceHtml}
        ${p.stock>0?`<div class="qty-ctrl"><button class="q-btn" onclick="cartDelta('${esc(p.name)}',-1,${activePrice})">−</button><span class="q-num" id="q_${safeName(p.name)}">${q}</span><button class="q-btn plus" onclick="cartDelta('${esc(p.name)}',1,${activePrice})">+</button></div>`:`<div style="text-align:center;font-size:.72rem;color:rgba(9,50,87,.33);padding:6px">نفاد المخزون</div>`}
      </div>`;
...
function cartGoStep3() {
  ...
  if (reviewBox) reviewBox.innerHTML = `
    ...
    ${items}
    <div style="display:flex;justify-content:space-between;padding:10px 0 4px;font-size:.92rem">
      <span style="font-weight:800;color:var(--deep)">الإجمالي</span>
      <span style="font-weight:900;color:var(--teal2);font-size:1.1rem">${total.toLocaleString()} د.ع</span>
    </div>
